...
```typescript
await page
  .frameLocator("#application-iframe")
...
const framePromise = page.waitForEvent("framenavigated", (frame) =>
...
**Key finding**: GPT-4V can successfully complete **51.1%** of tasks on live websites _if manually grounded_ (i.e., the human handles element selection). The main bottleneck is **grounding accuracy**, not understanding.
...
async function hybridInteract(page, naturalLanguageTarget: string) {
  ...
  const llmResponse = await askVisionLLM(
    screenshot,
  ...
  const domElement = await page.evaluate(
    ({ x, y }) => {
...
async function selectAntdOption(page, selectSelector: string, optionText: string) {
  ...
  const searchInput = page.locator(
    ".ant-select-dropdown .ant-select-search__field, input.ant-select-selection-search-input",
  );
...
await page
  .locator('.dz-hidden-input, input[type="file"][style*="visibility: hidden"]')
...
\* _Using `chrome.dom.openOrClosedShadowRoot`_
...
async function fillUnknownForm(page, formData: Record<string, string>) {
  ...
  const fieldMapping = await visionLLM.analyze(
    screenshot,
