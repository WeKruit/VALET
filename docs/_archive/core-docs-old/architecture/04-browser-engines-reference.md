...
const header = await stagehand.extract("extract the job title", z.object({ title: z.string() }), {
  selector: "//header",
});
...
// Multi-step action
await agent.act(
  "Select 'United States' from the country dropdown, then select 'California' from the state dropdown",
);
```
