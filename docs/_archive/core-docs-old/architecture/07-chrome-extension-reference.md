...
const FIELD_PATTERNS: Record<string, RegExp[]> = {
  firstName: [/first.?name/i, /fname/i, /given.?name/i, /prénom/i],
  lastName: [/last.?name/i, /lname/i, /surname/i, /family.?name/i, /nom/i],
  email: [/e?.?mail/i, /email.?address/i, /courriel/i],
  phone: [/phone/i, /tel/i, /mobile/i, /cell/i, /téléphone/i],
  linkedinUrl: [/linkedin/i, /linked.?in.?url/i, /linkedin.?profile/i],
  resume: [/resume/i, /cv/i, /curriculum/i, /attach/i],
  coverLetter: [/cover.?letter/i, /lettre.?de.?motivation/i],
  salary: [/salary/i, /compensation/i, /pay/i, /wage/i],
  startDate: [/start.?date/i, /available/i, /earliest/i, /when.*start/i],
  workAuthorization: [/authorized/i, /visa/i, /sponsorship/i, /work.?auth/i, /legally/i],
  yearsExperience: [/years?.?(?:of)?.?experience/i, /experience.?years/i],
  location: [/location/i, /city/i, /address/i, /where.*located/i],
  gender: [/gender/i, /sex/i],
  ethnicity: [/ethnic/i, /race/i, /demographic/i],
  veteran: [/veteran/i, /military/i],
  disability: [/disability/i, /handicap/i, /accommodation/i],
};
 ...
 */
export function fillReactCheckbox(element: HTMLInputElement, checked: boolean): boolean {
...
export function fillNativeSelect(select: HTMLSelectElement, value: string): boolean {
  ...
  if (!option) {
    ...
    const fuzzy = Array.from(select.options).find((o) =>
 ...
 */
export async function fillWorkdayDropdown(trigger: HTMLElement, value: string): Promise<boolean> {
 ...
 */
export function queryShadowDOM(root: Element, selector: string): Element | null {
...
async function handleStartFill(
  payload: StartFillMessage["payload"],
...
export default defineConfig({
  ...
  manifest: {
    ...
    version: "1.0.0",
    permissions: ["storage", "activeTab", "sidePanel", "alarms", "identity"],
    host_permissions: [
      "*://*.greenhouse.io/*",
      "*://boards.greenhouse.io/*",
      "*://jobs.lever.co/*",
