...
+-----------------------------------------------------------------------+
            ...
            v                       v                        v
+-------------------+  +---------------------+  +-----------------------+
|   apps/web        |  |   apps/api          |  |   Hatchet Engine      |
|   (Vite static)   |  |   (Fastify 5)       |  |   (Fly.io, AMQP)     |
|                   |  |                     |  |                       |
|   Port: 5173      |  |   Port: 8000        |  |   Workflow dispatch   |
+-------------------+  |                     |  |   + durable waits     |
                       |   - Auth (Google     |  +-----------+-----------+
                       |     OAuth + JWT)     |              |
                       |   - Task CRUD        |              | gRPC
...
- Task CRUD with Hatchet event dispatch (`task:created`) and cancel
...
**Role:** Hatchet workflow worker. Executes browser automation tasks.
...
```
User                    Web SPA              API Server           Hatchet            Worker              Browser Sandbox
 |                        |                     |                    |                  |                      |
 |-- POST /tasks -------->|                     |                    |                  |                      |
 |                        |-- POST /tasks ----->|                    |                  |                      |
 |                        |                     |-- INSERT task ---->|                  |                      |
...
```
User                Web SPA            API Server         Hatchet           Worker             Sandbox
 |                    |                    |                 |                  |                  |
 |                    |                    |                 |  check-captcha   |                  |
 |                    |                    |                 |  (durableTask)   |                  |
 |                    |                    |                 |  ============>   |                  |
...
```
User              Web SPA              API Server           Hatchet          Worker
 |                  |                     |                    |                |
 |-- Upload PDF --->|                     |                    |                |
 |                  |-- POST /resumes     |                    |                |
 |                  |   (multipart) ----->|                    |                |
...
|  |  (Hatchet client)|  |  (virtual display)|  |
|  |                  |  +-------------------+  |
|  |  - SandboxCtrl   |           |             |
|  |  - Stagehand     |  +-------------------+  |
|  |  - OrchAgent     |  |  x11vnc           |  |
...
**Constraints:**
- 1 session per worker machine (`slots=1` in Hatchet)
...
|
+-- valet-worker-{env}      Hatchet workflow worker
|   Config: fly/worker.toml
|   Machine: shared-cpu-2x, 1GB (MVP: includes browser)
|   Scaling: min=1, max=5
|   Ports: none (outbound only, except VNC in MVP)
|
+-- valet-hatchet-{env}     Hatchet engine (self-hosted)
|   Config: fly/hatchet.toml
|   Machine: shared-cpu-2x, 1GB
|   Scaling: min=1
|   Requires: CloudAMQP (RabbitMQ)
|
...
| Production | main      | https://valet-api.fly.dev      | https://valet-web.fly.dev     | https://valet-hatchet.fly.dev     |
...
**Deploy order (per environment):** Hatchet -> API -> Worker -> Web
...
| Workflows     | Hatchet           | Workflow orchestration               | gRPC (self-hosted)    |
| Message Queue | CloudAMQP         | Hatchet backend (RabbitMQ)           | AMQPS                 |
...
*This document is the authoritative architecture reference for WeKruit Valet. It should be updated as implementation progresses and design decisions evolve. Last updated: 2026-02-12.*
