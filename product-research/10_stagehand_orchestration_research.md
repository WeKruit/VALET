...
if (actualValue.value !== expectedValue) { /* retry or fallback */ }
```
...
```typescript
const errorElements = await stagehand.observe("find error messages, validation errors, or warning messages");
```
