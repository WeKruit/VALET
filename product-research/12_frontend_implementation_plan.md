...
@layer base {
  :root {
    ...
    --wk-border-strong: rgba(43, 24, 10, 0.20);
    ...
    --wk-shadow-lg: 0 8px 24px rgba(43, 24, 10, 0.10);
  ...
  [data-theme="dark"] {
    ...
    --wk-border-default: rgba(245, 230, 212, 0.10);
    ...
    --wk-shadow-sm: 0 1px 2px rgba(0, 0, 0, 0.20);
    --wk-shadow-md: 0 4px 12px rgba(0, 0, 0, 0.25);
    --wk-shadow-lg: 0 8px 24px rgba(0, 0, 0, 0.30);
  ...
  ],
  {
    variants: {
      variant: {
        ...
        ghost: [
          "bg-transparent text-wk-primary",
          "rounded-wk-lg",
          "hover:bg-wk-sunken",
        ],
        ...
        // Destructive action
        destructive: [
          "bg-wk-error text-wk-inverse",
          "rounded-wk-lg",
          "hover:opacity-90",
        ],
        // Link style
        link: [
          "text-wk-primary underline-offset-4",
          "hover:underline",
        ],
      },
      size: {
...
class WebSocketManager {
  ...
  private reconnect() {
    const { reconnectAttempts, incrementReconnect, setStatus } =
      useWebSocketStatus.getState();
  ...
  immer((set) => ({
    ...
    addField: (payload) =>
      set((state) => {
        const app = state.activeApplications.get(payload.applicationId);
        if (app) {
          const existingIndex = app.fields.findIndex(
...
export function useApplicationWebSocket() {
  ...
  useEffect(() => {
    ...
    const ws = new WebSocketManager(
      `${import.meta.env.VITE_WS_URL}/ws/applications`,
      token
    );
    ...
    <Dialog open modal>
      ...
      >
        ...
        <div className="p-wk-6 border-b border-wk-border-subtle">
          <DialogTitle className="wk-display-sm text-wk-primary">
            Attention Needed
          </DialogTitle>
        ...
        <div className="flex-1 bg-black relative overflow-hidden">
          <VncScreen
            ...
            loadingUI={
              <div className="flex items-center justify-center h-full">
                <div className="text-wk-inverse wk-body-sm">
          ...
          <div className="absolute bottom-4 left-4 right-4 text-center">
            <p className="wk-body-sm text-wk-inverse/80 bg-black/60 rounded-wk-md px-wk-4 py-wk-2 inline-block backdrop-blur">
              You are controlling the browser directly. Click and type normally
              to interact.
            </p>
          </div>
        </div>
...
*End of Frontend Implementation Plan. This document should be reviewed alongside 02_user_flows_and_ux_design.md and 05_autopilot_ux_onboarding.md for the complete design context. Implementation should begin with the Week 1 migration path described in Appendix B.*
