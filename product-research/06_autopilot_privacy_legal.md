...
**Copilot mode** does NOT meet this definition because the user reviews and approves each submission. The tool is a means of *assisting* the user, not acting independently.
...
**Autopilot introduces a nuance.** The question becomes: when a user authorizes an AI agent to act on their behalf, does the *agent's* access qualify as the *user's* authorized access?
...
**Autopilot risk:** HIGH. Submissions without per-action user review are squarely "automated methods to access the Services." LinkedIn cannot distinguish between a user who reviews each Easy Apply and one whose agent submits without review, but the *legal characterization* changes:
...
**Key consideration:** The "decision" in question is not the employer's hiring decision (which is the employer's Article 22 concern), but rather the decision to *submit a specific application with specific content*. When Autopilot makes this decision without human review:
...
*Consent version: v1.0 | Date: [auto-populated] | Your consent will be recorded with timestamp and session metadata.*
...
*This document is for internal planning and research purposes. All draft legal language must be reviewed by qualified legal counsel before use. This document does not constitute legal advice.*
