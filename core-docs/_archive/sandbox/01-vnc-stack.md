...
> **Note (updated 2026-02):** The MVP uses EC2 with systemd instead of Fly Machines with supervisord.
> The VNC component architecture (Xvfb → x11vnc → websockify → noVNC) remains identical,
> but process management uses systemd units on EC2 instead of supervisord in Docker.
> See [`03-ec2-worker-integration.md`](03-ec2-worker-integration.md) Section 11 for the EC2-specific
> systemd configuration. The React VNC components and security model in this doc remain valid.
