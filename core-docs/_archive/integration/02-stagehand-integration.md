...
>
> Stagehand v3 integration is deferred until the MVP (EC2 + Selenium + AdsPower) is stable.
> The MVP uses Selenium via AdsPower's `debuggerAddress` for browser automation.
> Stagehand will be integrated in V2 as the primary DOM-first automation engine,
> replacing Selenium for form analysis and intelligent field filling.
> See [`sandbox/03-ec2-worker-integration.md`](../sandbox/03-ec2-worker-integration.md) Section 16
> for the V2 session-based model that will use Stagehand.
