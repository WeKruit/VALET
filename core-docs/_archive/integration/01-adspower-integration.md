...
> **Update (2026-02):** AdsPower is now deployed on EC2 with headless mode (`--headless=true` CLI flag).
> The Local API requires a paid plan (Professional $9/mo+). Installation uses `.deb` package from
> `version.adspower.net`. First-time license activation requires VNC access to the EC2 instance.
> Browser profiles connect via Selenium `debuggerAddress` (not CDP directly for MVP).
> See [`sandbox/03-ec2-worker-integration.md`](../sandbox/03-ec2-worker-integration.md) Section 10
> for full EC2 AdsPower setup details and Section 14 for the integration mapping.
